...
using COPSBootstrapper.Elevated;

// COPSBootstrapper.Elevated — the only elevated part of the application (§20).
...
if (pipeName is null)
{
    Console.Error.WriteLine("Usage: COPSBootstrapper.Elevated --pipe <name>");
    return 2;
}

// Only accept locally generated pipe names to avoid being pointed at arbitrary pipes.
if (!pipeName.StartsWith("cops-elev-", StringComparison.Ordinal) || pipeName.Length > 64)
{
