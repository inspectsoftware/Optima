...
namespace COPSBootstrapper.App.Diagnostics;
...
public sealed class AdminPermissionsCheck : IDiagnosticCheck
{
    ...
    public Task<DiagnosticResult> RunAsync(CancellationToken ct = default)
    {
        ...
        return Task.FromResult(new DiagnosticResult
        {
            ...
            Status = status,
            Reason = reason,
            RecommendedFix = helperPresent ? string.Empty : "Reinstall or rebuild the application so COPSBootstrapper.Elevated.exe sits next to the main executable.",
        });
    }
}
