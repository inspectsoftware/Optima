...
namespace COPSBootstrapper.Platform.Windows.Elevation;
...
/// COPSBootstrapper.Elevated.exe via the shell "runas" verb (one UAC prompt), and exchanges
...
public sealed class ElevationBrokerClient : IElevationBroker
{
    ...
    public async Task<bool> EnsureStartedAsync(CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            var pipeName = "cops-elev-" + Convert.ToHexString(RandomNumberGenerator.GetBytes(16));
            ...
            var helperPath = Path.Combine(AppContext.BaseDirectory, "COPSBootstrapper.Elevated.exe");
