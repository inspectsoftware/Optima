...
public sealed class CopsException : Exception
{
    public UserFriendlyError Error { get; }

    public CopsException(UserFriendlyError error, Exception? inner = null)
        : base(error.Title, inner)
    {
        Error = error;
    }

    public static CopsException From(string code, string title, string explanation, Exception? inner = null, params string[] fixes)
